        ...
        <Body className="mx-auto w-[40rem] max-w-full bg-white px-5 font-sans">
          {website.emailHeaderImage && (
            <Section className="mb-5 text-center">
              <Img
                src={getAbsoluteImageUrl(website.emailHeaderImage, website.imageServerUrl)}
                alt="Seasea"
                className="block max-w-full mx-auto"
                style={{ minWidth: '400px', width: '100%', maxWidth: '600px', height: 'auto' }}
              />
            </Section>
          )}
          <Heading as="h1">
          ...
          <Section className="mt-10">
            <Row>
              <Column className="align-top pr-3" style={{ width: '18rem' }}>
              ...
              <Column className="align-top pl-3" style={{ width: '18rem' }}>
                <Text className="m-0 font-bold">
          ...
          <Section className="mb-10 mt-10">
            ...
            {productLineItems.map((item) => {
              // Check if pickup in store is selected
              const shippingFeeLine = rows.filter(
                (r) => r.rowType === 'SHIPPING_FEE'
              );
              ...
              const props = {
                ...
                stock: !isPickupInStore ? (stockDataMap?.[item.articleNumber] ?? null) : null,
                ...item,
              };
              return (
                <ProductLineItem
...
function ProductLineItem(props: any) {
  ...
  return (
    <Section className="my-2 border-b border-gray-200 pb-4" style={{ width: '100%', tableLayout: 'fixed' }}>
      <Row style={{ width: '100%' }}>
        <Column 
          className="align-top pr-4" 
          align="left" 
          style={{ 
            width: '280px', 
            verticalAlign: 'top',
        ...
        >
          <Text className="m-0 truncate text-sm uppercase" style={{ textAlign: 'left' }}>
            {product?.name}
          </Text>
          <Text className="m-0 truncate text-[10px] text-gray-500" style={{ textAlign: 'left' }}>
            Art. nr {articleNumber}
          </Text>
          {/* Stock status from huvudlager - only show if NOT pickup in store */}
          {stock && typeof stock.quantity === 'number' && (
            <Text className="m-0 mt-1 text-[11px] text-dark-gray">
              <span
                style={{
                  color: stock.quantity > 0 ? '#62D99B' : '#EF4444',
                  fontSize: '8px',
                  marginRight: '6px',
              ...
              {stock.quantity > 0
                ? `${translate('zs.inStock')} ${stock.quantity} ${translate('zs.stockCount') || ''}`
                : translate('zs.outOfStock')}
            </Text>
          )}
          {product.smallImages && product.smallImages.length > 0 && (
            <Text className="m-0 mt-2" style={{ textAlign: 'left', margin: 0, padding: 0 }}>
              <Img
